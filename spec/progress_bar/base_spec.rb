...
describe ProgressBar::Base do
  before do
    ...
    @progressbar = ProgressBar::Base.new(:output => @output, :length => 80, :throttle_period => 0.0)
  end
  ...
  context 'when a bar is started' do
    before do
      @progressbar = ProgressBar::Base.new(:starting_at => 0, :total => 100, :output => @output, :length => 80, :throttle_period  => 0.0)
    end
  ...
  context 'when the bar has not been completed' do
    before { @progressbar = ProgressBar::Base.new(:length => 112, :starting_at => 0, :total => 50, :output => @output, :throttle_period => 0.0) }
